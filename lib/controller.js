...
export function createController(ctx, deps = {}) {
  ...
  function markRestored(m) {
    ...
    if (restoredIds.size >= RESTORED_IDS_CAP) restoredIds.clear()
    restoredIds.add(String(m.id))
  }
  ...
  // stashed. Best-effort: a disposed agent simply drops them. Each restored
  // message is marked first: the re-insert fires `agent/inbox/inserted`
  // synchronously, and the restore must not be mistaken for fresh input.
