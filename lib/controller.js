...
import {
  ...
  overCap, remainingMs, errorTextOf, classifyStop, isRateLimited, boundaryHasContent,
} from './pure.js'
...
export function createController(ctx, deps = {}) {
  ...
  function onInboxInserted(payload) {
    ...
    if (m == null || typeof m !== 'object' || !isHoldable(m)) return
    try {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (action === 'failure') {
      ...
      const entry = sessions.get(String(id))
      // A content-less completed boundary (no claimed prompt, no output, no
      // usage) is a scheduler artifact — exactly what the driver closes while
      // queued prompts are being held. It is NOT a success: keep waiting.
      if (entry && entry.holding && !boundaryHasContent(event)) return
