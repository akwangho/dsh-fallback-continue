...
import { DEFAULTS, normalizeConfig, intervalFor, capStopText, rateLimitedText, formatStopTime, overCap, remainingMs, errorTextOf, classifyStop, isRateLimited } from './pure.js'

export { normalizeConfig, intervalFor, DEFAULTS, capStopText, rateLimitedText, formatStopTime, overCap, remainingMs, errorTextOf, classifyStop, isRateLimited }
...
function createController(ctx) {
  ...
  // Rate-limit (HTTP 429) stop: do NOT arm the continue loop — retrying would
  // only deepen the limit — but leave a visible in-session notice explaining
  // why "繼續" was NOT sent, then drop any in-flight streak. The notice
  // carries the stop time so the user knows when the 429 halt happened and
  // can estimate how long to wait before manually continuing.
  function rateLimitStop(sessionId) {
    recordNotice(sessionId, rateLimitedText(now()))
    const e = sessions.get(String(sessionId))
    if (e) stopEntry(String(sessionId), 'rate-limited')
  }
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (action === 'failure') {
      // API 429 (rate limit) is terminal for unattended mode: a retry would
      // just deepen the limit, so stop and explain instead of arming "繼續".
