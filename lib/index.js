...
function createController(ctx) {
  ...
  return {
    ...
    updateConfig(patch) {
      ...
      if (typeof p.continueText === 'string' && p.continueText.length > 0) config.continueText = p.continueText
      if (typeof p.capEnabled === 'boolean') config.capEnabled = p.capEnabled
      if (typeof p.capHours === 'number' && p.capHours >= 0) config.capHours = p.capHours
      ...
      return { ok: true, config: snapshot().config }
    },
    pause(sessionId) {
      const id = String(sessionId)
