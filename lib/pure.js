 ...
 *   - 'blocked'              -> 'failure' (the attempt was rejected before the
 *     turn ran — e.g. capacity/rate guards dropped the claimed input; the
 *     streak must re-arm or it would wait forever on a dead turn)
 *   - 'completed'            -> 'reset'   (streak resets)
 *   - 'aborted'              -> 'reset'   (the turn was cancelled — a human
 *     stop must end the loop; a later failure re-arms it if needed)
 *   - anything else          -> 'ignore'
 */
export function classifyStop(kind) {
  if (kind === 'error' || kind === 'max-tokens' || kind === 'blocked') return 'failure'
  if (kind === 'completed' || kind === 'aborted') return 'reset'
  return 'ignore'
}

/**
 * Whether a message was authored by the human user (`source.kind === 'user'`,
 * as stamped by the session prompt RPC). Such a message arriving while a
 * failure streak is active is an explicit takeover: the human is present and
 * driving, so the loop must stop — never silently stash their input away.
 */
export function isUserAuthored(m) {
  const src = m && m.source
  return !!(src && src.kind === 'user')
}

/**
 * Whether a completed `turn/end` event carries evidence that the turn really
 * did something: claimed input, model usage, or an explicit step flag.
 *
 * While queued prompts are being held, the driver still closes occasional
 * content-less boundary turns (wake/convergence artifacts with nothing left
 * to claim). Counting one of those as success would release the held prompts
 * while the task is still broken — so they must be recognized and ignored.
 * The check is deliberately over-inclusive (any plausible activity field
 * counts as content) so a harness version that omits one of them can never
 * make a REAL completed turn look empty.
 */
export function boundaryHasContent(event) {
  const data = event && event.data
  if (data == null || typeof data !== 'object') return true // unknown shape: assume content
  // A truncated (max-tokens) turn obviously did work.
  const kind = data.reason && data.reason.kind
  if (kind === 'max-tokens') return true
  // Claimed input under any plausible field name.
  for (const key of ['messages', 'claimed', 'claimedMessages', 'inputs', 'input', 'prompts']) {
    const v = data[key]
    if (Array.isArray(v) && v.length > 0) return true
    if (v != null && !Array.isArray(v) && typeof v === 'object') return true
  }
  // Step / model-call evidence.
  for (const key of ['steps', 'stepCount', 'requestCount', 'modelCalls']) {
    const v = Number(data[key])
    if (Number.isFinite(v) && v > 0) return true
  }
  if (data.hadStep === true || data.stepped === true || data.ranModel === true) return true
  // Token usage of any shape (flat or nested) implies a model call ran.
  for (const u of [data.usage, data.tokenUsage, data.usageSummary]) {
    if (u != null && typeof u === 'object') {
      for (const k of ['totalTokens', 'inputTokens', 'outputTokens', 'promptTokens', 'completionTokens', 'total_tokens', 'input_tokens', 'output_tokens']) {
        const v = Number(u[k])
        if (Number.isFinite(v) && v > 0) return true
      }
    }
