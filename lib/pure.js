 ...
 * Includes the stop time so the user knows when the 429 halt happened and can
 * estimate how long to wait before manually continuing.
 */
export function rateLimitedText(at = Date.now()) {
  return '⛔ 自動繼續未觸發：最後一次失敗來自 API 429（Rate Limit，請求過於頻繁，發生時間：' + formatStopTime(at) + '）。重試只會持續觸發限流，因此此會話不再自動送出「繼續」。請稍候再手動送出訊息（可對照上述時間判斷已等待多久），或調整模型／金鑰的速率限制。'
}
