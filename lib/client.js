    ...
    const SettingsPage = (props) => {
      ...
      return h('div', { className: 'fbc-settings' },
        ...
        h('div', { className: 'fbc-field' },
          ...
          h('input', { className: 'fbc-input', type: 'number', min: 1, value: String(draft.cooldownMinutes), onChange: (ev) => setDraft({ ...draft, cooldownMinutes: ev.target.value }) }),
          h('button', { className: 'fbc-btn', onClick: () => save({ cooldownMinutes: num(draft.cooldownMinutes) }) }, t('save')),
        ),
        waitList(state, h),
          ...
          return h('div', { key: e.sessionId, className: 'fbc-list-row' },
            h('div', { className: 'fbc-list-main' },
              ...
              h('div', { className: 'fbc-list-sub' },
                '#' + (e.failures + 1) + ' · ' + phaseText + (e.cooling ? ' · ' + t('cooling') : '') + ' · ' + (e.reason || 'error')),
            ),
