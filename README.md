...
- **排隊提示詞保留**：只要自動繼續循環在等待中（倒數中或已送出「繼續」），之後排入的提示詞會先被**保留**（從 agent 的 inbox 暫時移出），不會在「繼續」之前被執行；直到該輪**成功**（turn 以 `completed` 且有實際內容結束）才按原本的先後順序（FIFO）放回排隊。這樣排隊中的提示詞不會在上一個任務尚未完成時就搶先開跑。取消、使用者接管、關閉外掛等其他停止方式也會把保留的提示詞放回，不會遺失；唯有 archive 會連同排隊訊息一起取消。
